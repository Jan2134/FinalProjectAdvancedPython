...
class TestFilteringMethods(unittest.TestCase):
    def setUp(self):
        self.df = pd.DataFrame(
            {"column1": [1, 2, 3, 4, 5], "column2": [10, 20, 30, 40, 50]}
        )
    ...
    def test_bigger_than(self):
        ...
        np.testing.assert_array_equal(filtered_df.values, expected_result.values)

    def test_smaller_than(self):
        filtered_df = Filtering.smaller_than(self.df, "column2", 30)
        expected_result = pd.DataFrame({"column1": [1, 2], "column2": [10, 20]})
